    ...
    public int dfs(int node, int parent, Map<Integer, List<Integer>>adj, List<Boolean> hasApple){
        ...
        if(!adj.containsKey(node))
            ...
            for(int child: adj.get(node)){
                ...
                //Childtime >0 indicates subtree of child has apples. since the root node of the subtree does not contibute to the time, even if it has an apple, we have to check it independently
